...
} from '@/electron/main/runtime/agent-runtime';
import type { DuneAcpOptions } from '@/electron/main/runtime/dune-agent';
import {
  resolveAgentDuneDir,
...
import type { DuneChannel } from '@/electron/main/runtime/dune-channel';
...
describe('AgentRuntime', () => {
  ...
  it('treats completed one-shot assignment tasks as missing for sweep checks', async () => {
    ...
    const mockAgent = harness.mockAgent();
    const getTask = mockAgent.getTask as unknown as (taskId: string) => { status?: string } | undefined;
  ...
  it('records workflow activity when a scheduled task starts', async () => {
    const homeDir = createTempHome();
    const harness = createAgentLiteModuleHarness();
    let workflowSnapshot: WorkflowSnapshot = {
      items: [
        {
    ...
    const onItemActivityChanged = vi.fn();

    tempDirs.push(homeDir);

    const host = new AgentRuntime({
      actionServices: {
        getRuntimeController: () => ({}) as never,
        onWorkflowChanged,
        workflowStore,
      },
      agentStore: createMemoryStore(),
    ...
    const item = workflowSnapshot.items[0];

    expect(onItemActivityChanged).toHaveBeenCalledWith({ itemId: 'item-1', isWorking: true });
    expect(onWorkflowChanged).toHaveBeenCalledTimes(1);
    expect(item).toMatchObject({
      activity: expect.objectContaining({ totalEventCount: 1 }),
      updatedAt: 42,
    });
    expect(item?.workflowEvents[0]).toMatchObject({
      actor: 'Assignment Agent',
      createdAt: 42,
      description: 'Started working on this item.',
      kind: 'item',
    });
  });
